...
function RenderGalaxy(arr, xoffset, yoffset) {
  ...
  for (var y = ymin; y < ymax; y++) {
    for (var x = xmin; x < xmax; x++) {
      ...
      var result = $.grep(galaxyarr, function (e) {
        return parseInt(e.x) == x;
      });
      if (result.length > 0) {
        var res = $.grep(result, function (e) {
          return parseInt(e.y) == y;
        });
        if (res.length > 0) {
          RenderGalaxyBlock(centerx, centery, res);
        } else {
          RenderGalaxyBlock(centerx, centery, [
            { x: x, y: y, type: "unexplored" },
          ]);
        }
      } else {
        RenderGalaxyBlock(centerx, centery, [
...
function RenderGalaxyBlock(x, y, arr) {
  ...
  if (arr[0].type == "planet") {
    ...
    $("#pos_" + x + "_" + y)
      .removeClass("cell_undiscovered")
      .addClass("cell_planet_" + type);
    ...
    $("#pos_" + x + "_" + y).click(function () {
      SetPlanetModal(arr[0].x, arr[0].y);
    });
  }
  if (arr[0].type == "explored") {
    $("#pos_" + x + "_" + y)
      .removeClass("cell_undiscovered")
      .addClass("cell_discovered");
    $("#pos_" + x + "_" + y).attr("data-target", "#DiscoveredCoordModal");
    $("#pos_" + x + "_" + y).click(function () {
      SetDiscoveredModal(arr[0].x, arr[0].y);
    });
  }
  if (arr[0].type == "explore") {
    ...
    $("#pos_" + x + "_" + y)
      .removeClass("cell_undiscovered")
      .addClass("cell_discover");
    if (arr[0].user == username) {
      $("#pos_" + x + "_" + y).addClass("myexploration");
    }
    $("#pos_" + x + "_" + y).click(function () {
      SetUndiscoveredModal(arr[0].x, arr[0].y);
    });
  }
  ...
  if (!ZoomClears[calczoom]) {
    ...
    for (var y = 1; y <= height; y++) {
      ...
      for (var x = width; x > 0; x--) {
        ...
        var data =
          '<div class="cell_undiscovered" id="pos_' +
          nx +
          "_" +
          ny +
          '" data-toggle="modal" style="height:' +
          bh +
          "px;width:" +
          bh +
          "px; background-size: " +
          ...
          "px " +
          bh +
          'px;" data-target="#UndiscoveredCoordModal"></div>';
  ...
  GalaxyArray.forEach(function (res) {
    if (res[0].x == x && res[0].y == y) {
      ...
      newText = replaceAll(
        newText,
        "date_",
        moment(res[0].date * 1000).format("LL")
      );
      newText = replaceAll(
        newText,
        "date2_",
...
function SetUndiscoveredText(res, ship, callback = null) {
  ...
  if (ship) {
    ...
    ureturn = moment(parseInt(ttime) + parseInt(ttime) + unix).format(
      "L, hh:mm:ss"
    );
  ...
  activeMission(function (err, ares) {
    ...
    if (!btnclickund && x != null && y != null) {
      undbtn = $("#und_modal_btn").one("click", function () {
        btnclickund = true;
        $("#UndiscoveredCoordModal").data("bs.modal").options.backdrop =
          "static";
...
function SetPlanetModal(x, y) {
  activeMission(function (err, ares) {
    if (ares.mission_allowed) avipl = 1;
    else {
      avipl = 0;
    }
        ...
        $.ajax({
          ...
          success: function (msg) {
            ...
            if (json.planet_type == "earth") {
              ...
              type =
                json.planet_type.charAt(0).toUpperCase() +
                json.planet_type.slice(1);
            }
            ...
            var planetname =
              json.planet_name.charAt(0).toUpperCase() +
              json.planet_name.slice(1);
            ...
            newText = replaceAll(
              newText,
              "date_",
              moment(json.planet_crts * 1000).format("LL")
            );
