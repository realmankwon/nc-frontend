        ...
        success: function (response) {
          ...
          $.ajax({
            ...
            success: function (msg) {
              ...
              $("#fleet_available").html(
                "Available Missions User: " +
                  userUnused +
                  " / " +
                  userMissionsMax +
                  ", Planet: " +
                  ...
                  " / " +
                  planetMissionsMax +
                  ' (<span class="infocolor"><a class="infocolor" href="buffs.php">Buffs</a></span>)'
              );
              if (!missionpossible) {
                NextStep("main", 0);
              }
                  ...
                  msg.forEach((mis) => {
                    if (
                      ...
                      if (undersiege) {
                        $("#missionselector")
                          .empty()
                          .append("<option>Choose a mission</option>")
                          .append("<option>Break Siege</option>");
                        $("#ehancemc").css("display", "block");
                        $("#ehancemc").text(
                          "You can't start a mission. Break the siege first."
                        );
                      }
                    }
                  });
...
function SelectMainFleet(value) {
  ...
  if (missionpossible) {
    ...
    if (value == "Explore") {
      if (
        (ShipTypes.includes("explorership") ||
          ShipTypes.includes("explorership1")) &&
        missionpossible
      ) {
      ...
      value == "Attack" &&
      missionpossible &&
      attacktypestrs.some(function (ff) {
        return ShipTypes.indexOf(ff) > -1;
      })
    ) {
      if (ShipTypes.length > 0) {
        $("#fleet_main_btn").prop("disabled", false);
...
function RenderSelectFleet() {
  ...
  ships.forEach(function (type) {
    if (ShipTypes.includes(type)) {
      ...
      if (doRender) {
        var inputField =
          '<input autocomplete="off" id="field_' +
          index +
          '" onkeyup="TotalSelected()" type="text" class="form-control" placeholder="" style="font-size:12px; color:#ffffff; background-color:transparent;width:50%;float:right;">';
        $("#fleet_select_table")
          .append("<tbody>")
          .append(
            $("<tr>")
...
function RenderSendFleet(json) {
  ...
  if (json.time) {
    ...
    var ret = moment(parseInt(json.time) + parseInt(json.time) + unix).format(
      "L, hh:mm:ss"
    );
  ...
  if (FirstChoice == "support") {
    ...
    CheckCords(function (err, res) {
      if (
        parseInt(json.diff) > 0 &&
        parseFloat(uraniumqytu) >= con &&
        powner != username
      ) {
        $("#fleet_send_btn").prop("disabled", false);
      } else {
  ...
  if (posx != null && posy != null) {
    ...
    $.ajax({
      ...
      success: function (msg) {
        ...
        if (json.type == "planet") {
          text +=
            "Explored (Planet: " + json.name + ", Owner: " + json.user + ")";
          powner = json.user;
...
function RenderLoadFleet() {
  ...
  for (var i = 0; i < 4; i++) {
    ...
    var inputField =
      '<input autocomplete="off" id="input_' +
      res +
      '" onkeypress="validateint(event); CapCheck(event)" onkeyup="CheckResLoad()" type="text" class="form-control" placeholder="" style="font-size:12px; color:#ffffff; background-color:transparent;width:50%;float:right;">';
    $("#load_table")
      .append("<tbody>")
      .append(
        $("<tr>")
  ...
  $("#load_info").text(
    "Utilization/Capcity of your fleet: " + 0 + "/" + totalCapa
  );
  CheckResLoad();
}
...
function CheckResLoad() {
  ...
  if (total <= totalCapa) {
    $("#load_info").html(
      "Utilization/Capacity of your fleet: " + total + "/" + totalCapa
    );
    ...
    $("#load_info").html(
      'Utilization/Capacity of your fleet: <span class="text-dangerq">' +
        total +
        "/" +
        totalCapa +
        "</span>"
    ...
    MakeAPhonecall(
      ...
      function (err, res) {
        if (res) {
          ...
          $("#" + btnid).html("Start Mission");
          alert(
            "No response from Backend, Custom_JSON was send \n\n" +
...
function NextStep(next, step) {
  if (
    (FirstChoice == "explore" && step == 3) ||
    (FirstChoice == "attack" && step == 3) ||
    (FirstChoice == "siege" && step == 3) ||
    (FirstChoice == "support" && step == 3) ||
    (FirstChoice == "breaksiege" && step == 3)
  ) {
    StartMission();
  } else {
...
function RenderPlanets() {
  ...
  if (
    ...
    $.ajax({
      ...
      success: function (msg) {
        ...
        msg.forEach(function (data) {
          if (
            data.type == "attack" &&
            !oldattackmis.includes(data.to_planet.id)
          ) {
            if (data.to_planet.user != username) {
              if (crst < 5) {
                $("#planet_selection").attr(
                  "onchange",
                  "FleetPlanet(null,value)"
                );
                $("#planet_selection").append(
                  '<option value="' +
                    data.end_x +
                    "/" +
                    data.end_y +
    ...
    planet_ids.forEach(function (id, index) {
      $("#planet_selection").append(
        '<option value="' + id + '">' + planet_names[index] + "</option>"
      );
