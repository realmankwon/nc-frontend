...
function UpgradeHandler(name, json) {
  ...
  if (!isNaN(busytime) && busytime > utc) {
    ...
    htmltimer =
      '<div style="text-align: center; color: #ffffff; font-size:12px;" id="Timer_' +
      name +
      '">Next Upgrade available in ' +
      UpgradeTimestr +
      "</div>";
  }
  ...
  if (json.base <= baselevel || json.research <= researchlevel) {
    if (
      parseFloat(oreqytu) >= json.ore &&
      parseFloat(coalqytu) >= json.coal &&
      parseFloat(copperqytu) >= json.copper &&
      parseFloat(uraniumqytu) >= json.uranium
    ) {
      if (makeUpgrade == false) {
        var dis = "";
        if (!isNaN(busytime) && busytime > utc) {
        ...
        document.getElementById("upg_" + name).innerHTML =
          htmltimer +
          '<button type="button" class="btn btn-success btn-sm btn-block" id="upg_btn_' +
          name +
          '" onclick="Upgrade(\'' +
          name +
          "')\" " +
      ...
      document.getElementById("upg_" + name).innerHTML =
        htmltimer +
        '<button type="button" class="btn btn-default btn-sm btn-block" disabled="disabled">Miss Resources</button>';
    }
    ...
    html.innerHTML =
      htmltimer +
      '<button type="button" class="btn btn-danger btn-sm btn-block" disabled="disabled">Upgrade ' +
      text +
      "</button>";
  }
}
...
function UpgradeTimer() {
  ...
  handlernames.forEach(function (res) {
    if (!isNaN(res.time) && res.time > utc) {
      ...
      document.getElementById("Timer_" + res.name).innerHTML =
        "Next Upgrade available in " + UpgradeTimestr;
    }
  });
}
