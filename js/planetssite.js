  ...
  $.ajax({
    ...
    success: function (msg) {
      ...
      if (json.planet_type == "earth") {
        ...
        type =
          json.planet_type.charAt(0).toUpperCase() + json.planet_type.slice(1);
      }

      bonus =
        json.planet_rarity.charAt(0).toUpperCase() +
        json.planet_rarity.slice(1);
      if (bonus == "Undefined") {
        bonus = "Common";
      }
...
function DataBuilder(dataJson) {
  ...
  LoadTransactions({ user: owner, limit: 200 }, function (err, res) {
    if (res) {
      ...
      var tr_type = "";
      if (res[0]) {
        DateString = moment(res[0].date * 1000).format(
      ...
      //Get Items
      LoadItems(
        { planetid: showcaseID, user: owner, limit: 200 },
        function (err, res) {
          //if(res){
          var chest = 0,
            blueprint = 0;
                      ...
                      function (err, res) {
                        ...
                        LoadData(
                          ...
                          function (err, res) {
                            ...
                            //Load Fleet
                            LoadData(
                              { planetid: showcaseID, user: owner },
                              "loadfleet",
                              function (err, res) {
                                var ordered = {};
                                res.forEach((el) => {
                                ...
                                LoadData(
                                  ...
                                  function (err, res) {
                                    ...
                                    res.forEach((el) => {
                                      ...
                                      if (
                                        el.arrival >= currentUTC ||
                                        el.return >= currentUTC
                                      ) {
                                        activehold[el.arrival] = el;
                                        if (
                                          el.from_planet.id == showcaseID ||
                                          el.type == "deploy" ||
                                          el.type == "support"
                                        ) {
                                          Object.keys(el.ships).forEach(
                                            function (key) {
                                              if (key != "total") {
                                                if (ships[key]) {
                                                  ships[key] =
                                    ...
                                    console.log(ships);
                                    const ordered = {};
                                    Object.keys(allmissions)
                                      .reverse()
                                      .forEach(function (key) {
                                        ordered[key] = allmissions[key];
                                      });
                                    dataJson["fleetmission"] = {
                                      active: activehold,
                                      all: ordered,
                                      ships: ships,
                                    };
...
function LastRenderer(data) {
  ...
  $("#mapref").attr(
    "href",
    "/galaxy.php?y=" + planet.planet_cory + "&x=" + planet.planet_corx
  );
  ...
  Object.keys(productions.unfiltered).forEach(function (key) {
    if (key != "misc") {
      ...
      if (ressources[key + "depot"] < resam) {
        ...
        timefull =
          ((ressources[key + "depot"] - resam) / ressources[key + "rate"]) *
            86400 +
          utc;
      }
      var percent = ((curObj.safe * 100) / ressources[key + "depot"]).toFixed(
        2
      );
      ...
      $("#t_ressources")
        ...
        .append(
          $("<tr>")
            ...
            .append(
              $("<td>").append(
                '<span id="' + key + '_res">' + resam.toFixed(2) + "</span>"
              )
            )
            ...
            .append(
              $("<td>").append(
                "+" + ressources[key + "rate"].toFixed(1) + "/day"
              )
            )
            .append(
              $("<td>").append(curObj.safe.toFixed(2) + " (" + percent + "%)")
            )
        );
      ...
      ClientResUpdate(
        ressources[key],
        ressources[key + "depot"],
        ressources["lastUpdate"],
        ressources[key + "rate"],
        key
      );
    }
  ...
  Object.keys(buildings).forEach(function (key) {
    ...
    $("#t_buildings")
      ...
      .append(
        $("<tr>")
          .append(
            $("<td>").append(
              Translate(curObj.name) + " (lvl. " + curObj.current + ")"
            )
          )
          .append($("<td>").append(state))
          .append(
            $("<td>").append(
              '<span id="' + buildings[key].name + '_time">-</span>'
            )
          )
          ...
          .append(
            $("<td>").append(
              '<span id="' + buildings[key].name + '_stime">-</span>'
            )
          )
      );

    TickTook(
      buildings[key].name + "_time",
      curObj.busy,
      function (err, res) {
        console.log(err, res);
    ...
    TickTook(
      buildings[key].name + "_stime",
      skills[curObj.name].busy,
      function (err, res) {
        console.log(err, res);
    ...
    $("#t_ships")
      ...
      .append(
        $("<tr>")
          ...
          .append($("<td>").append(state))
          .append(
            $("<td>").append('<span id="' + curObj.type + '_time">-</span>')
          )
          .append($("<td>").append(skill))
          .append($("<td>").append(sstate))
          .append(
            $("<td>").append('<span id="' + curObj.type + '_stime">-</span>')
          )
      );
...
function loadmore() {
  //Load Missions
  LoadData(
    { planetid: showcaseID, user: owner, limit: 25, page: page },
    "loadfleetmission",
    function (err, res) {
      var allmissions = {};
      res.forEach((el) => {
    ...
    .forEach(function (key) {
      ...
      LoadPlanetData(curObj.from_planet, function (err, res) {
        ...
        LoadPlanetData(curObj.to_planet, function (err, res) {
          ...
          var sym =
            '<img src="/img/arrow-left.png" height="8px" style="margin-left:5px;">';
          var color = "white";
          if (curObj.from_planet.id == showcaseID) {
            sym =
              '<img src="/img/arrow-right.png" height="8px" style="margin-left:5px;">';
          } else {
            if (
              curObj.type == "attack" ||
              curObj.type == "siege" ||
              curObj.type == "breaksiege"
            ) {
              color = "#a94442";
            }
          ...
          var typetip = "";
          if (
            curObj.resources.coal > 0 ||
            curObj.resources.ore > 0 ||
            curObj.resources.uranium > 0 ||
            curObj.resources.copper > 0
          ) {
            typetip = "<span>";
          ...
          if (curObj.battles > 0) {
            activestatus =
              '<a href="/report?mission=' +
              curObj.id +
              '" style="font-size:13px; text-decoration: none; color:' +
              statuscolor +
              ';">' +
              status +
              "</a>";
            ...
            activestatus =
              '<a style="font-size:13px; text-decoration: none; color:' +
              statuscolor +
              ';">' +
              status +
              "</a>";
          ...
          if (curObj.result == "planet_found") {
            ...
            var plnfnd =
              ...
              "</span>";
            activestatus =
              '<a class="tooltips" style="text-decoration: none; color:' +
              statuscolor +
              '; font-size:13px;"">' +
              status +
              plnfnd +
          ...
          if (curObj.new_stardust != null) {
            var end =
              "<span><b>Stardust found</b><br>The crew found a small amount of Stardust during the exploration.<br><br>+ " +
              curObj.new_stardust / 100000000 +
              " Stardust</span>";
            activestatus =
              '<a class="tooltips" style="text-decoration: none; color:#72bcd4; font-size:12px;"">' +
              curObj.new_stardust / 100000000 +
              " SD" +
              end +
              "</a>";
          ...
          if (curObj.new_item_id != null) {
            var end =
              "<span><b>Blueprint found</b><br>The crew found a blueprint during the exploration. This blueprint was found:<br><br>+ " +
              blueprint[curObj.new_item_id] +
              "</span>";
            activestatus =
              '<a class="tooltips" style="text-decoration: none; color:#add8e6; font-size:12px;"">' +
              "Blueprint" +
              end +
              "</a>";
          }
          ...
          $("#t_missions")
            ...
            .append(
              $("<tr>")
                .append(
                  $("<td>").append(
                    '<a class="tooltips" style="color: ' +
                      color +
                      '; text-decoration:none">' +
                      Translate(curObj.type) +
                      typetip +
                      "</a>" +
                      sym
                  )
                ...
                .append(
                  $("<td>").append(
                    '<a class="tooltips" style="text-decoration:none" onclick="LoadGalaxyA(\'' +
                      curObj.start_x +
                      "','" +
                      curObj.start_y +
                      "')\"> [" +
                ...
                .append(
                  $("<td>").append('<span id="' + key + '_atime">-</span>')
                )
                .append(
                  $("<td>").append(
                    moment(curObj.arrival * 1000).format("DD/MM hh:mm:ss")
                  )
                )
                .append(
                  $("<td>").append('<span id="' + key + '_rtime">-</span>')
                )
