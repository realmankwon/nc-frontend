...
function loadMarketData() {
  $.ajax({
    ...
    success: function (data) {
      console.log(data);
      var total =
        data.items_on_market + data.planets_on_market + data.ships_on_market;
...
function RenderFilter(category = "all", subcategory = "all") {
    ...
    if (
      ...
      if (category != "all" && category != item.category) {
        ...
        $("#subcategory_filter").append(
          $("<option></option>")
            .val(item.subcategory)
            .html(capitalize(item.subcategory.replace("booster", "rune")))
        );
      }
    }
    // Type
      ...
      if (category != "all" && category != item.category) {
        ...
        if (category != "planet") {
          if (subcategory != "all" && subcategory != item.subcategory) {
            ...
            $("#type_filter").append(
              $("<option></option>").val(item.type).html(item.name)
            );
          }
        } else {
          $("#type_filter").append(
  ...
  if (category == "planet") {
    $("#subcategory_filter").append(
      $("<option></option>").val("1").html("Common")
    );
    $("#subcategory_filter").append(
      $("<option></option>").val("2").html("Uncommon")
    );
    $("#subcategory_filter").append(
      $("<option></option>").val("3").html("Rare")
    );
    $("#subcategory_filter").append(
      $("<option></option>").val("4").html("Legendary")
    );
  }
...
function RenderHistory(dataarray) {
  ...
  his.forEach(function (data) {
    ...
    console.log(data);
    $("#market_history_tbody").append(
      $("<tr>")
        .append(
          $("<td>").append(
            moment(data.sold * 1000).format("MMMM D, YYYY [at] h:mm:ss a")
          )
        )
  ...
  dataarray.forEach(function (data, index) {
    ...
    if (data.category == "planet") {
      ...
      tmp_ = tmp_.replace(
        "img_",
        "/img/planets/" +
          PlanetRawToIMG(data.type, data.subcategory, data.img_id) +
          ".png"
      );
      tmp_ = tmp_.replace(
        "info1_",
        PlanetIDToName("special", data.subcategory) +
          " / " +
          PlanetIDToName("type", data.type)
      );
      tmp_ = tmp_.replace(
        "info2_",
        "Coordinate: " + data.cords_hor + "/" + data.cords_ver
      );
    }
    ...
    if (data.subcategory == "blueprint") {
      ...
      tmp_ = tmp_.replace(
        "info1_",
        '<img src="img/icons/ok3.png" width="11px" style="margin:0 3px 3px 0;"><span style="color:#add8e6;">Blueprint</span>'
      );
    }
    ...
    if (item_name.includes("Chest")) {
      ...
      tmp_ = tmp_.replace(
        "info2_",
        'Resources <a class="tooltips" href="" style="text-decoration:none;font-size:11px;"><img src="img/icons/info.png" width="13px" height="13px" style="margin:-2px 0 0 0;"><span>' +
          info[data.type] +
          "</span></a>"
      );
    }
    ...
    if (data.user != username) {
      tmp_ = tmp_.replace(
        "btntext_",
        '<span class="glyphicon glyphicon-tag" aria-hidden="true" style="color:#FFF;"></span> Buy now'
      );
    ...
    tmp_ = tmp_.replace(
      "click_",
      "OpenMarketModal('" + modaltype + "'," + extramodal + ")"
    );
