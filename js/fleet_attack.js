...
function FilterDropdown() {
  ...
  attacktypes.forEach(function (ship, index) {
    $("#attack_sel_" + index).html(
      '<option value="1">' + chooseTxt + "</option>"
    );
    attacktypestrs.forEach((ship) => {
      var fleeti = ShipTypes.indexOf(ship);
      if (fleeti >= 0) {
        total = ShipTotal[fleeti];
        $("#attack_sel_" + index).append(
          $("<option>", {
            value: ship,
...
function MaxSelectAttack(event, type) {
  ...
  formationqyt[form] = 0;
  var sum = formationqyt.reduce(function (pv, cv) {
    return pv + cv;
  }, 0);
  if (sum == 0) {
