...
function pickApiServer() {
  if (
    ...
    location.hostname === "140.82.34.132"
  ) {
...
function customJsonToken(account, id, json, cb) {
  ...
  api.customJson(["active"], [account], id, json, function (err, res) {
    ...
    if (lap == planetid) {
      if (err) {
        window.open(
          "https://www.steemconnect.com/sign/custom-json?id=" +
            id +
            "&json=" +
            json +
            '&authority=active&required_posting_auths=[]&required_auths=["' +
            account +
            '"]',
          "_blank"
        );
  ...
  setInterval(function () {
    $.ajax({
      ...
      success: function (data) {
        data.forEach((line) => {
          if (
            line.tr_type == json.type &&
            line.user == username &&
            line.block_num == block &&
            line.tr_status == 1
          ) {
