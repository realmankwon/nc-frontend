  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (limit == 100 && type == null) {
        ...
        $("#selectsell").html("");
        $("#selectsell").append(
          $("<option>", { value: "all" }).text("Type: All")
        );
        ...
        optiondata.forEach(function (ship) {
          ...
          if (!optionfilter.includes(ship.type)) {
            $("#selectsell").append(
              $("<option>", { value: ship.type }).text(shipname)
            );
...
function sortedShips(data) {
  ...
  sortedShips = _.chain(sortedShips)
    .sortBy("type")
    .sortBy("shipyard_level")
    .sortBy("for_sale")
    .value();
  ...
  fleetdata.forEach(function (ship) {
    ...
    if (ship.for_sale == 0) {
      lastLine =
        ...
        ship.id +
        '_sd" type="text" class="form-control input-sm" placeholder="Enter SD price..." style="font-size:12px; color:#ffffff; background-color:transparent;"><span class="input-group-btn"><button class="btn btn-info btn-sm" id="' +
        ship.id +
        '_sell" type="button">Sell</button></span></div>';
    }
      ...
      $("<tr>")
        .append(
          $("<td>").append(
            '<img src="img/ships/' +
              img +
              '.png" width="40px" style="margin: 0 5px 0 0;"> ' +
              shipname
          )
        )
...
function SellShip(id) {
  ...
  scJson["tr_var3"] = parseFloat($("#" + id + "_sd").val());
