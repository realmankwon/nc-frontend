...
var overhalf = {};
var info =
  "You can cancel this mission now, but then the required uranium for the flight will be lost. Are you sure you want to do this now?<br><br>";
var deployif =
  '<span style="color:#a94442;"><i>Your fleet already has flown more than half the way and therefore not enough uranium for the way back. This mission can not be canceled.</i></span>';
...
function sortAfter(type, sort) {
  ...
  if (sort != "ships") {
    ...
    data = data.sort((a, b) =>
      a.ships.total > b.ships.total ? 1 : b.ships.total > a.ships.total ? -1 : 0
    );
  }
...
function loadHeader() {
  activeMission(function (err, res) {
    if (res) {
      if (missionplanet) {
        missionplanetname =
          "(" + planet_names[planet_ids.indexOf(missionplanet)] + ")";
      } else {
        missionplanetname = "";
      }
...
function RenderMissions(data, id, append = null) {
  ...
  data.forEach((mission) => {
    ...
    //Tooltip Logic
    if (
      mission.resources.coal > 0 ||
      mission.resources.ore > 0 ||
      mission.resources.uranium > 0 ||
      mission.resources.copper > 0
    ) {
      type_tooltip = "<span>";
    ...
    var report = "";
    var reports =
      '<a style="font-size:12px;" target="_blank" href="/report?mission=' +
      mission.id +
      '">Log1</a> / <a target="_blank" style="font-size:12px;" href="' +
      jarunikServer +
      "/battle/" +
      mission.id +
      '">Log2</a>';
    ...
    if (mission.from_planet.user == username) {
      if (!mission.cancel_trx) {
        cancel =
          '<a style="font-size:12px;" id="cancel_' +
          mission.id +
          ...
          .append(
            $("<td>").append(
              '<a class="tooltips" style="text-decoration:none">' +
                mission.ships.total +
                "<span>" +
                loadShipData(mission) +
                "</span></a>"
            )
          )
          .append(
            $("<td>").append(
              moment(mission.start_date * 1000).format("DD/MM hh:mm:ss")
            )
          )
...
function preparetimer(mission, id, time) {
  ...
  if (!time) {
    ...
    if (utc < time) {
      ...
      activestatus[mission.id + id] =
        '<a class="tooltips" id="timer_' +
        mission.id +
        id +
        '" style="text-decoration:none">' +
        tis;
      TickTook(
        "timer_" + mission.id + id,
        parseInt(time),
        function (err, res) {
          if (res) {
            if (id == "arr") {
...
function statusbuilder(mission) {
  ...
  if (!result) {
    ...
    if (result == "cancel") {
      span =
        '<a class="tooltips" style="color:#dad986; text-decoration:none; font-size:13px;">Aborted<span>Mission was aborted</span></a>';
    }
    ...
    if (mission.result == 0 || mission.result == 1 || mission.result == 2) {
      span =
        ...
        mission.id +
        '" style="font-size:12px; text-decoration: none; color:' +
        statuscolor +
        ';">' +
        status +
        "</a>";
    ...
    if (mission.new_stardust != null) {
      var end =
        ...
        mission.new_stardust / 100000000 +
        " Stardust</span>";
      span =
    ...
    if (result == "planet_found") {
      ...
      var plnfnd =
        "<span><b>Planet found</b><br>Rarity: " +
        mission.to_planet.bonus +
        "<br>Type: " +
        typ +
        "<br>Position: " +
        mission.end_x +
        ...
        "</span>";
      span =
        '<a class="tooltips" style="text-decoration: none; color:#5CB85C; font-size:12px;">Planet' +
        plnfnd +
        "</a>";
    }
...
function activeMission(callback) {
  ...
  $.ajax({
    ...
    success: function (msg) {
      ...
      callback(null, {
        cur: json.user_unused,
        max: json.user_max,
        planet_unused: json.planet_unused,
        planet_max: json.planet_max,
        mission_allowed: json.mission_allowed,
      });
    },
  });
}
...
function CancelMission(mission, hold) {
  ...
  scJson["tr_type"] = "cancel";
  scJson["tr_var1"] = mission;
