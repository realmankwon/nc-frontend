  ...
  for (var i = 0; i < ships.length; i++) {
    ...
    if (found_ship) {
      ...
      if (cur_obj.variant_name == "yamato") {
        ...
        type +=
          ' / <img src="img/icons/rocket.png" width="13px" height="13px" style="margin:0 6px 0 0;">Rocket: ' +
          cur_obj.rocket;
        type +=
          ' / <img src="img/icons/laser.png" width="13px" height="13px" style="margin:0 6px 0 0;">Laser: ' +
          cur_obj.laser;
      ...
      temp_div = replaceAll(
        temp_div,
        "sys_",
        redss + cur_obj.shipyard_min_level + "</span>"
      );
      temp_div = replaceAll(
        temp_div,
        "syb_",
        reds + cur_obj.shipyard_min_level + "</span>"
      );
      ...
      CostBuilder += RessourceBuilder(
        cur_obj.costs.coal,
        cur_obj.costs.ore,
        cur_obj.costs.copper,
        cur_obj.costs.uranium
      ).cost;
      ...
      if (cur_obj.activated == false && buyable[cur_obj.type]) {
        ...
        if (buyable[cur_obj.type] == 2 && !boughtships.includes(cur_obj.type)) {
          ...
          temp_div = temp_div.replace(
            "onclick_",
            "window.open('https://guide.nextcolony.io/reward', '_blank')"
          );
          temp_div = temp_div.replace("disabled_", "");
        }
        if (buyable[cur_obj.type] == 2 && boughtships.includes(cur_obj.type)) {
      ...
      if (cur_obj.ship_skill < 20) {
        ...
        temp_div = temp_div.replace(
          "btn_",
          Translate("enhance") + " " + cur_obj.class + " Skill"
        );
...
function Yamato() {
  ...
  $.ajax({
    ...
    success: function (json) {
      ...
      $("#tiersel").html(
        '<option value="">Choose a tier to upgrade...</option>'
      );
      Object.keys(yamatos)
        .sort()
        .forEach((yamato) => {
          ...
          var style = "00";
          var tiers = (
            style.substring(0, style.length - tier.length) + tier
          ).toString();
          if (tiers != "20") {
            $("#tiersel").append(
              $("<option>", {
        ...
        success: function (data) {
          ...
          $("#tiersel").on("change", function () {
            ...
            if (val.length > 0) {
              ...
              var style = "00";
              var tier =
                style.substring(0, style.length - tiero.length) + tiero;
