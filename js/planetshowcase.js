...
function SetBurnData(json, bonus, type) {
  ...
  $.ajax({
    ...
    success: function (njson) {
      ...
      njson.forEach((data) => {
        if (
          data.bonus == nbonus.toString() &&
          data.planet_type == ntype.toString()
        ) {
...
function loadPlanetShowcase(id, path) {
  ...
  $.ajax({
    ...
    success: function (msg) {
      ...
      if (path == "planet") {
        document.getElementById("planet_detail").innerHTML =
          "# " + id + " / " + bonus + " / " + type;
      } else {
      ...
      var sell = selltext
        .replace("name_", json.planet_name)
        .replace("ref_", "/planet/" + json.planet_id)
        .replace("id_", json.planet_id)
        .replace("corx_", json.planet_corx)
        .replace("cory_", json.planet_cory)
        .replace("username_", username)
        .replace("rare_", bonus)
        .replace("type_", type)
        .replace("discovered_", date);
      $("#sellinfo").html(sell);
