...
function RenderBuffs() {
  buffs.forEach(function (buff, index) {
    $("#buff" + index + "_price").html(
      formatNumber(buff.price / 100000000) + " Stardust"
    );
    ...
    var activeBuffs = Math.ceil(
      (buff.buff_end - currentUTC) / (buff.buff_duration * 86400)
    );
    if (activeBuffs <= 0) {
      activeBuffs = 0;
    }
    ...
    tmpbuff = tmpbuff.replace(
      "date_",
      moment(buff.buff_end * 1000).format("MMMM D, YYYY [at] h:mm:ss a")
    );
