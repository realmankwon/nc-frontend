...
function Production() {
  $("#coalprdo").text(coalprd ? coalprd.production.toFixed(2) : 0);
  $("#oreprdo").text(oreprd ? oreprd.production.toFixed(2) : 0);
  $("#copperprdo").text(copperprd ? copperprd.production.toFixed(2) : 0);
  $("#uraniumprdo").text(uraniumprd ? uraniumprd.production.toFixed(2) : 0);
}
...
function Shipyard() {
  ...
  $.ajax({
    ...
    success: function (json) {
      ...
      json.forEach(function (building) {
        if (
          building.busy_until > currentUTC &&
...
function Market() {
  $.ajax({
    ...
    success: function (data) {
      ...
      $("#tsold").text(
        formatNumber((data.trading_volume / 100000000).toFixed(0)) + " Stardust"
      );
      $("#tburned").text(
        formatNumber((data.total_burned / 100000000).toFixed(0)) + " Stardust"
      );
      $("#hsell").text(
        formatNumber((data.highest_sale.price / 100000000).toFixed(0)) +
          " Stardust"
      );

      var hinfo =
        "<b>Highest sale</b><br>Category: " +
        capitalize(data.highest_sale.category) +
        "<br>Price: " +
        formatNumber((data.highest_sale.price / 100000000).toFixed(0)) +
        " SD<br>Seller: " +
        data.highest_sale.seller;
      $("#hinfo").html(hinfo);
    },
  });
}
...
function Mission() {
  ...
  $.ajax({
    ...
    success: function (json) {
      ...
      $("#ava_missions").text(
        maxmissions - json.own_missions + " / " + maxmissions
      );
      ...
      json.forEach(function (mission) {
        if (cur < 10) {
          ...
          if (mission.new_stardust != null) {
            ...
            template = template.replace(
              ...
              '<span class="stardust">' +
                ...
                " Stardust</span> found on exploration to [" +
                mission.end_x +
                "/" +
                mission.end_y +
          ...
          if (mission.new_item_id != null) {
            ...
            template = template.replace(
              ...
              '<span style="color:#add8e6;">' +
                ...
                " Blueprint</span> found on exploration to [" +
                mission.end_x +
                "/" +
                mission.end_y +
          ...
          if (mission.new_planet_id != null) {
            ...
            template = template.replace(
              "notificationcontent_",
              '<span style="color:#5CB85C;">Planet</span> found! ' +
                capitalize(mission.to_planet.bonus) +
                " " +
                capitalize(planet_type) +
                " on position [" +
                mission.end_x +
                "/" +
                mission.end_y +
                ']. <a href="galaxy.php">Galaxy</a> »'
            );
            $("#notification").append(template);
          }
        }
...
function BattleOfTheDay() {
  ...
  $.ajax({
    ...
    success: function (json) {
      ...
      $("#battle_t").text(
        moment(json.loot_date * 1000).format("MM/D/YY, h:mm:ss a")
      );

      $("#battle_off").attr("href", "/report?mission=" + json.loot_mission);
      $("#battle_jar").attr(
        "href",
        jarunikServer + "/battle/" + json.loot_mission
      );
    },
  });
}
