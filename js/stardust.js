    ...
    success: function (njson) {
      ...
      if (!check) {
        njson.transactions.forEach(function (data) {
          ...
          if (from != username) {
            amountHTML =
              '<span class="stardust">' + formatNumber(amount) + "</span>";
          } else {
            amountHTML =
              '<span class="text-dangerq">' + formatNumber(amount) + "</span>";
          }
          ...
          if (trx) {
            trxtd =
              '<a href="' +
              apiServer +
              "/loadtransaction?trx_id=" +
              data.trx +
              '" >' +
              data.trx.substring(0, 8) +
              "...</a>";
          } else {
            trxtd = data.trx;
          }
...
function ConvertSDModal() {
  ...
  $("#mbin1").text(
    "Balance: " + formatNumber(Math.floor(balance)) + " Stardust"
  );
  $("#mbex1").text(
    "Balance: " + formatNumber(Math.floor(balance_se)) + " Stardust"
  );
...
function GiftSDCJ(
  ...
  scJson["tr_type"] = "transferstardust";
  scJson["tr_var1"] = Math.floor(amount);
  scJson["tr_var2"] = user;
