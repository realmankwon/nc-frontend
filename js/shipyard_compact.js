      ...
      shiporder.forEach(function (key) {
        ...
        var d = data.filter(function (item) {
          if (!found && item.type == key) {
            ...
            if (
              nextBuild != null &&
              item.busy_until < nextBuild &&
              item.busy_until > currentUTC &&
              nextBuild != item.busy_until
            ) {
              nextBuild = item.busy_until;
            }
...
function ShipyardBuilder(shipyardData) {
  ...
  shipyardData.forEach((ship) => {
    ...
    var scoal = ship.costs.coal,
      sore = ship.costs.ore,
      suranium = ship.costs.uranium,
      scopper = ship.costs.copper,
      sstardust = 0;

    if (coalqyt < ship.costs.coal) {
      scoal = '<span class="text-dangerq">' + ship.costs.coal + "</span>";
      buildable = false;
    }
    if (oreqyt < ship.costs.ore) {
      sore = '<span class="text-dangerq">' + ship.costs.ore + "</span>";
      buildable = false;
    }
    if (copperqyt < ship.costs.copper) {
      scopper = '<span class="text-dangerq">' + ship.costs.copper + "</span>";
      buildable = false;
    }
    if (uraniumqyt < ship.costs.uranium) {
      suranium = '<span class="text-dangerq">' + ship.costs.uranium + "</span>";
      buildable = false;
    }
    if (ship.costs.stardust != null) {
      sstardust = ship.costs.stardust / 100000000;
    }
    if (stardustt < ship.costs.stardust / 100000000) {
      sstardust =
        '<span class="text-dangerq">' +
        ship.costs.stardust / 100000000 +
        "</span>";
      buildable = false;
    }
    ...
    //Busy
    var busy =
      '<svg style="display:none" fill="currentColor" width="19" height="19" viewBox="0 0 24 24" class="material-design-icon__svg"><path d="M6,2H18V8H18V8L14,12L18,16V16H18V22H6V16H6V16L10,12L6,8V8H6V2M16,16.5L12,12.5L8,16.5V20H16V16.5M12,11.5L16,7.5V4H8V7.5L12,11.5M10,6H14V6.75L12,8.75L10,6.75V6Z"><title>Transaction sent</title></path></svg>' +
      "<span id='timer_" +
      ship.type +
      "'>-</span>";
    if (ship.busy_until > 0) {
      ...
      if (!now.isAfter(busyUntil)) {
        busy =
          ...
          ship.type +
          "'>" +
          moment.duration(now.diff(busyUntil)).humanize() +
          "</span>";
        buildable = false;
      }
    }
    ...
    if (buildable && !refreshIds.includes(ship.type)) {
      build_btn =
        '<button id="btn_' +
        ship.type +
        '" class="btn btn-success btn-xs buildbutton">Build</button>';
    }
    ...
    $("#shipyard").append(
      $("<tr>")
        ...
        .append(
          $("<td>").append(
            moment.duration(parseInt(ship.costs.time), "seconds").humanize()
          )
        )
...
function sortAfter(sort) {
  ...
  if (
    ...
    data = data.sort((a, b) =>
      a.costs[sort] > b.costs[sort] ? 1 : b.costs[sort] > a.costs[sort] ? -1 : 0
    );
  } else if (sort == "AD") {
    data = data.sort((a, b) =>
      a.rocket + a.bullet + a.laser > b.rocket + b.bullet + b.laser
        ? 1
        : b.rocket + b.bullet + b.laser > a.rocket + a.bullet + a.laser
        ? -1
        : 0
    );
  } else {
