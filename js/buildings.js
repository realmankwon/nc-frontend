...
function RenderBuildings() {
  ...
  for (var i = 0; i < buildings.length; i++) {
    ...
    if (cur_obj) {
      ...
      if (cur_obj.name == "shieldgenerator") {
        ...
        if (shieldcharge_busy > currentUTC) {
          temp_div = temp_div.replace(
            "charingtext_",
            "Charging in progress, finish in "
          );
          ...
          temp_div = temp_div.replace(
            "charingtext_",
            "Charging time: " + msToHMS(chargetime * 1000)
          );
        ...
        if (shieldprotection_busy > currentUTC) {
          ...
          TickTook(
            cur_obj.name + "protectiontimer",
            shieldprotection_busy,
            function (err, res) {
              if (res) {
                AwaitChangeBuildings();
              }
      ...
      CostBuilder += RessourceBuilder(
        cur_obj.coal,
        cur_obj.ore,
        cur_obj.copper,
        cur_obj.uranium
      ).cost;
      enough = RessourceBuilder(
        cur_obj.coal,
        cur_obj.ore,
        cur_obj.copper,
        cur_obj.uranium
      ...
      if (cur_obj.name.includes("mine")) {
        ...
        LoadProductionData(function (err, res) {
          if (
            production_misc.type == res &&
            production_misc.bonus != "common"
          ) {
      ...
      if (cur_obj.cur_rate) {
        var prd_builder =
          '<p style="display: dis_prd; font-size: 13px">Current Level: ' +
          cur_obj.cur_rate +
          " " +
          res;
        if (cur_obj.next_rate) {
          prd_builder =
            prd_builder + "<br>Next Level: " + cur_obj.next_rate + " " + resl;
        }
            ...
            if (index == res) {
              ...
              if (el.booster > 0) {
                prd_builder =
                  prd_builder +
                  br +
                  '<span style="font-size:13px;">Production Increase <a href="skills.php">Skill</a> (lvl. ' +
                  el.booster +
                  ') <span style="color:#5cb85c;">+' +
                  prc +
                  "%</span>: " +
                  ((prc / 100) * cur_obj.cur_rate).toFixed(2) +
                  " " +
                  Translate(res) +
                  " / Day</span>";
                totalres += parseFloat(
              ...
              if (prddata.misc.type == res && prddata.misc.rate > 0) {
                prd_builder =
                  prd_builder +
                  br +
                  '<span style="font-size:13px;">Rarity Boost <a class="tooltips" href="" style="text-decoration:none;font-size:11px;"><img src="img/icons/info.png" width="14px" height="14px" style="margin:-3px 0 0 0;"><span>Based on the rarity your planet gets a boost to the associated type. These are the possible rarity boost levels in ascending order: Uncommon (10%), Rare (20%), Legendary (100%).</span></a> (' +
                  capitalize(prddata.misc.bonus) +
                  ') <span style="color:#5cb85c;">+' +
                  prddata.misc.rate +
                  "%</span>: " +
                  dayrarity.toFixed(2) +
              ...
              if (prddata.misc.rune > 0 && prddata.misc.type == res) {
                prd_builder =
                  prd_builder +
                  br +
                  '<span style="font-size:13px;">Rune Boost <a class="tooltips" href="" style="text-decoration:none;font-size:11px;"><img src="img/icons/info.png" width="14px" height="14px" style="margin:-3px 0 0 0;"><span>The rune boost supports the rarity boost. There are three different runes in the shop. These are the possible gradations: Rune (20%), Mighty Rune (50%), Holy Rune (100%).</span></a> (' +
                  prddata.misc.rune_name +
                  ') <span style="color:#5cb85c;">+' +
                  prddata.misc.rune +
                  "%</span>: " +
                  dayrune.toFixed(2) +
              ...
              if (totalres > cur_obj.cur_rate) {
                prd_builder =
                  prd_builder +
                  '<p style="font-size:13px; margin:10px 0 0 0;"><b>Total Production:</b> ' +
                  totalres.toFixed(2) +
                  " " +
                  Translate(res) +
                  " / Day</p>";
        ...
        if (cur_obj.name.includes("depot")) {
          ...
          $.each(prddata, function (index, el) {
            if (index == res) {
              ...
              if (
                prddata.misc.type != index ||
                prddata.misc.bonus == "common"
              ) {
              } else {
                daytotal += dayrarity;
              ...
              if (
                prddata.misc.type != index ||
                (prddata.misc.bonus == "common" && prddata.misc.type == 0)
              ) {
              } else {
                daytotal += dayrune;
                hourtotal += hourrune;
              ...
              var TimeNeeded =
                ((parseInt(el.depot) - CurrentRessource) / hourtotal) * 3600000;
              ...
              if (depotskill > 0) {
                prd_builder =
                  prd_builder +
                  '<span style="font-size:13px;">Depots Capacity Increase <a href="skills.php">Skill</a> (lvl. ' +
                  depotskill +
                  ') <span style="color:#5cb85c;">+' +
                  toFixed(depotskill * 2.5, 2) +
                  "%</span>: " +
                  depotu.toFixed(2) +
                  " " +
                  capitalize(Translate(res)) +
                  "</span>";
              }
              if (researchlevel > 0) {
                if (depotskill > 0) {
                ...
                prd_builder =
                  prd_builder +
                  '<span style="font-size:13px;">Research Center (lvl. ' +
                  researchlevel +
                  ') <span style="color:#5cb85c;">+' +
                  toFixed(researchlevel * 2.5, 2) +
                  "%</span>: " +
                  ru.toFixed(2) +
                  " " +
                  capitalize(Translate(res)) +
              ...
              var fullin =
                ' (full in: <span id="depot_' + res + '_timer"></span>)';
              ...
              prd_builder =
                prd_builder +
                '<p style="font-size:13px; margin:10px 0 0 0;"><b>Total Capacity</b><span id="depot_' +
                res +
                '">' +
                fullin +
                "</span>: " +
              ...
              prd_builder =
                prd_builder +
                '<div style="border-bottom: 1px solid rgba(255, 255, 255, 0.20);margin:16px 0 14px 0;"></div>';
              ...
              prd_builder =
                prd_builder +
                '<span style="font-size:13px;"><span style="color:#5cb85c;">Safe:</span> ' +
                toFixed(el.safe, 2) +
                " " +
                capitalize(Translate(res)) +
                " (" +
                toFixed(safep, 2) +
                "% of the total capacity)</span>";
            }
          });
        }
      ...
      if (cur_obj.name.includes("bunker")) {
        ...
        var protection = 0;
        var sprotection = 5 + level * 0.5;
        var nprotection = 5 + (level + 1) * 0.5;
        protection = sprotection + bunkerskill * 0.25;

        var prd_builder =
          '<p style="display: dis_prd; font-size: 13px">Protection Current Level: ' +
          sprotection.toFixed(2) +
          "%";
        prd_builder =
          prd_builder +
          "<br>Protection Next Level: " +
          nprotection.toFixed(2) +
          "%";
        prd_builder = prd_builder + "</p>";
        ...
        if (bunkerskill > 0) {
          prd_builder =
            prd_builder +
            '<span style="font-size:13px;">Bunker Protection Increase <a href="skills.php">Skill</a> (lvl. ' +
            bunkerskill +
            '): <span style="color:#5cb85c;">+' +
            bunkerskill * 0.25 +
            "%</span></span>";
        ...
        prd_builder =
          prd_builder +
          '<p style="font-size:13px; margin:10px 0 0 0;"><b>Total Protection:</b> ' +
      ...
      if (cur_obj.skill < level + 1) {
        ...
        temp_div = temp_div.replace(
          "btn_",
          Translate("enhance") + " " + Translate(cur_obj.name) + " Skill"
        );
        temp_div = temp_div.replace(
          "onclick_",
