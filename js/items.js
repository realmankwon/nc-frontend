...
function RenderItems() {
  ...
  for (var i = 0; i < result.length; i++) {
    ...
    if (cur_obj != null) {
      ...
      if (cur_obj.coal) {
        ...
        temp_div = temp_div.replace(
          "2line_",
          cur_obj.booster + "% " + Translate("booster")
        );
      ...
      temp_div = temp_div.replace(
        "total_",
        cur_obj.total - itemOnSaleCount(cur_obj.id)
      );
  ...
  GetMyPlanets(function (err, res) {
    ...
    planet_ids.forEach(function (id, index) {
      ...
      $("#items_chest_modal").append(
        '<option value="' + id + '">' + planet_names[index] + "</option>"
      );
      $("#items_booster_modal").append(
        '<option value="' + id + '">' + planet_names[index] + "</option>"
      );
      $("#items_blueprint_modal").append(
        '<option value="' + id + '">' + planet_names[index] + "</option>"
      );
    });
  });
...
function MergeItem() {
  $("#items_merge_modal_btn").prop("disabled", true);
  $("#items_merge_modal_btn").html(
    '<img src="img/loading.gif" height="15px" />'
  );
  $("#blueprint_merge_modal_btn").prop("disabled", true);
  $("#blueprint_merge_modal_btn").html(
    '<img src="img/loading.gif" height="15px" />'
  );
